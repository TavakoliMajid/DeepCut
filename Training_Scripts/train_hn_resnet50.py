...
from sklearn.metrics import precision_recall_fscore_support, confusion_matrix, log_loss
...
# FIXED PATHS
# ============================
DATA_ROOT = Path(r"D:\\cologne\\HyperNetwork\\Data")
INIT_BACKBONE_PATH = Path(r"D:\\cologne\\Models\\Models\\Resnet50_big.pth")
OUTDIR = Path(r"D:\\cologne\\HyperNetwork\\Nueral_Network\\resnet50_hn")

TRAIN_DIR = DATA_ROOT / "train"
VAL_DIR = DATA_ROOT / "val"
TEST_DIR = DATA_ROOT / "test"

TRAIN_META = DATA_ROOT / "train_metadata.json"
VAL_META = DATA_ROOT / "val_metadata.json"
TEST_META = DATA_ROOT / "test_metadata.json"
...
# ---- Eval / calibration ----
MIN_PRECISION = 0.95       # stricter precision target
USE_TTA = True            # off by default for stability

# ---- Uncertainty band (off by default) ----
USE_UNCERTAIN_BAND = False
UNCERTAIN_BAND = 0.05

# ---- Label smoothing for BCE (small) ----
LABEL_SMOOTH = 0.05        # set 0.0 to disable

# ---- EMA ----
USE_EMA = True
EMA_DECAY = 0.999

# ---- Phase-2 FT (adjusted LR and patience) ----
ENABLE_PHASE2 = True
PHASE2_LR = 4e-5
PHASE2_EPOCHS = 20

# Globals for metadata normalization
META_MEAN = None
META_STD  = None
...
def count_pos_weight(labels: np.ndarray) -> float:
    ...
    return 1.0 if pos==0 else max(1.0, float(neg)/float(pos))
...
def load_from_folder_and_metadata(dir_path: Path, meta_path: Path) -> list:
    if not meta_path.exists():
        raise FileNotFoundError(f"Metadata file not found: {meta_path}")
    metadata = json.load(open(meta_path, "r"))
    items = []
    for subdir in ["good_cut", "bad_cut"]:
        subdir_path = dir_path / subdir
        if subdir_path.exists():
            for img_path in subdir_path.glob("*.png"):
                image_name = img_path.name
                if image_name in metadata:
                    label = 1 if subdir == "bad_cut" else 0
                    items.append({
                        "image_path": str(img_path),
                        "label": label,
                        "metadata": build_metadata_list(metadata[image_name]),
                    })
    return items
...
class JsonDataset(Dataset):
    def __init__(self, items: List[Dict[str,Any]], img_size:int=384, augment:bool=True):
        self.items = items
        pil_augs = []
        if augment:
            pil_augs = [
        ...
        tensor_augs = []
        if augment:
            tensor_augs = [transforms.RandomErasing(p=0.10, scale=(0.02, 0.06), ratio=(0.3, 3.3))]
    ...
    def __getitem__(self, i):
        ...
        if hasattr(self, 'augment') and self.augment:
            meta_arr += np.random.normal(0, 0.1, meta_arr.shape).astype(np.float32)  # Reduced metadata variation
        y = torch.tensor(it["label"], dtype=torch.float32)
        meta = torch.tensor(meta_arr, dtype=torch.float32)
        return x, y, meta

# ============================
# Model (ResNet50 + HyperNet FiLM)
# ============================
class FeatureResNet50(nn.Module):
    def __init__(self, pretrained=True):
        super().__init__()
        try: m = models.resnet50(weights=None)
        except TypeError: m = models.resnet50(pretrained=False)
...
class HyperNet(nn.Module):
    def __init__(self, meta_dim:int, out_channels:int=2048, hidden:int=128, dropout:float=0.5):
        super().__init__()
...
class HyperFiLMResNet50(nn.Module):
    def __init__(self, meta_dim:int, freeze_early:bool=False, head_dropout:float=0.3):
        super().__init__()
        self.backbone = FeatureResNet50(pretrained=True)
        self.hypernet = HyperNet(meta_dim=meta_dim, out_channels=self.backbone.out_channels, hidden=128, dropout=0.5)
    ...
    def forward(self, x, meta):
        ...
        f = self.adapter(f,g,b); x = self.gap(f).flatten(1)
        return self.head(x).squeeze(1)

# ============================
# Load ResNet50 conv features
# ============================
def load_backbone_features_from_ckpt(model: HyperFiLMResNet50, ckpt_path: Path):
    ...
    for k,v in sd.items():
        kk = k.replace("module.","")
        if kk.startswith(("conv1","bn1","layer1","layer2","layer3","layer4")):
            new_sd[f"backbone.features.{kk}"] = v
        if kk.startswith("backbone.") and any(blk in kk for blk in ["conv1","bn1","layer1","layer2","layer3","layer4"]):
            new_sd["backbone.features."+kk.split("backbone.",1)[1]] = v
        if kk.startswith("features.") and any(blk in kk for blk in ["conv1","bn1","layer1","layer2","layer3","layer4"]):
            new_sd["backbone."+kk] = v
...
def evaluate(model, loader, device, threshold: float = 0.5, temperature: float = 1.0, use_tta: bool=False):
    ...
    for xb,yb,mb in loader:
        ...
        if use_tta:
            logits = torch.stack([model(x_aug, mb) for x_aug in _tta_variants(xb)],0).mean(0)
        else:
            logits = model(xb, mb)
    ...
    return {"precision":p, "recall":r, "f1":f1, "cm":cm, "nll":nll, "probs":probs, "targets":targets, "logits":logits}
...
def search_threshold_for_precision(probs: np.ndarray, targets: np.ndarray, min_precision=0.95):
    ...
    # if never reaches min_precision, fall back to max-F1
    if best_recall < 0:
...
# Plotting
# ============================
def plot_metric_curves(epochs, tr_prec, tr_rec, tr_f1, va_prec, va_rec, va_f1, save_path: Path):
    plt.figure(figsize=(9,6))
    plt.plot(epochs, tr_prec, marker='o', label='Train Precision')
    plt.plot(epochs, tr_rec, marker='o', label='Train Recall')
    plt.plot(epochs, tr_f1, marker='o', label='Train F1')
    plt.plot(epochs, va_prec, marker='s', label='Val Precision')
    plt.plot(epochs, va_rec, marker='s', label='Val Recall')
...
def plot_confusion_matrix(cm: np.ndarray, title: str, save_path: Path):
    ...
    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            ax.text(j, i, format(cm[i, j], 'd'), ha="center", va="center", color="white" if cm[i, j] > thr else "black")
...
def main():
    ...
    # ---- Load splits with metadata ----
    train_items = load_from_folder_and_metadata(TRAIN_DIR, TRAIN_META)
    val_items   = load_from_folder_and_metadata(VAL_DIR, VAL_META)
    test_items  = load_from_folder_and_metadata(TEST_DIR, TEST_META)
    if len(train_items)==0 or len(val_items)==0 or len(test_items)==0:
        raise RuntimeError("One of the splits is empty after parsing. Check folder structure/paths.")
    ...
    model = HyperFiLMResNet50(meta_dim=meta_dim, freeze_early=False, head_dropout=HEAD_DROPOUT).to(device)
    ...
    # ---- History ----
    hist = {"epoch": [], "train_p": [], "train_r": [], "train_f1": [], "val_p": [], "val_r": [], "val_f1": []}
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        for k,src in [("train_p","precision"),("train_r","recall"),("train_f1","f1")]:
            hist[k].append(float(train_metrics[src]))
        for k,src in [("val_p","precision"),("val_r","recall"),("val_f1","f1")]:
            hist[k].append(float(val_metrics[src]))

        print(f"Epoch {epoch:03d} | lr={sched.get_last_lr()[0]:.2e} | "
              f"Train F1={train_metrics['f1']:.3f} P={train_metrics['precision']:.3f} R={train_metrics['recall']:.3f} | "
              f"Val F1={val_metrics['f1']:.3f} P={val_metrics['precision']:.3f} R={val_metrics['recall']:.3f} NLL={val_metrics['nll']:.3f}")
    ...
    if ENABLE_PHASE2 and patience <= 0:
        print("[Phase 2] Starting fine-tuning with lower LR")
        for param in model.parameters():
            param.requires_grad = True
        opt = torch.optim.AdamW(model.parameters(), lr=PHASE2_LR, weight_decay=WEIGHT_DECAY)
        sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=PHASE2_EPOCHS)
        patience = PATIENCE

        for epoch in range(EPOCHS+1, EPOCHS+PHASE2_EPOCHS+1):
            model.train()
            for xb,yb,mb in train_dl:
                xb,yb,mb = xb.to(device), yb.to(device), mb.to(device)
                opt.zero_grad(set_to_none=True)
            ...
            if val_metrics["f1"] > best_val_f1 + 1e-4:
                ...
                patience -= 1
                if patience <= 0:
                    print("Phase 2 early stopping."); break
    ...
    # ---- Uncertainty band stats (optional) ----
    ...
    with open(OUTDIR/"summary.json","w") as f:
        json.dump({
            ...
            "use_tta": USE_TTA,
            "val": {k:(float(v) if not hasattr(v,"tolist") else v.tolist()) for k,v in val_eval.items() if k in ["precision","recall","f1","nll"]},
            "test": {k:(float(v) if not hasattr(v,"tolist") else v.tolist()) for k,v in test_metrics.items() if k in ["precision","recall","f1","nll"]},
    ...
    # ---- Plots ----
    plot_metric_curves(hist["epoch"], hist["train_p"], hist["train_r"], hist["train_f1"],
                       hist["val_p"], hist["val_r"], hist["val_f1"], OUTDIR/"train_val_metrics_over_epochs.png")
    ...
    print(f"[Test ] F1={test_metrics['f1']:.3f}  P={test_metrics['precision']:.3f}  R={test_metrics['recall']:.3f}")
